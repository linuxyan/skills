...
- **⚠ 重大教训：双次分红制度导致全年分红率严重低估（东阿阿胶案例，2026-05-06）**

  > **教训来源**：东阿阿胶（000423.SZ）2024年和2025年均实施了"中报+年报"双次分红。
  >
  > **错误经过**：
  > - 原报告仅查询并使用了年报分红（10派12.73元、10派14.31元）
  > - 完全遗漏了中报分红（10派11.47元、10派12.70元）
  > - 导致2024年分红率从100.0%被错误计算为47.3%
  > - 导致2025年分红率从100.0%被错误计算为53.0%
  >
  > **后果**：分红率低估导致g_base计算偏高（从≈0修正为≈0，结论未大幅翻转），但若出现在其他公司可能造成严重错误
  >
  > **根因**：API返回的分红记录按"公告日期"排列，而非"归属年度"。当API返回某年8-9月的中报分红时，易误认为该行是年报分红
  >
  > **正确做法（必须严格遵守）**：
  > 1. **分别查询中报分红和年报分红**：
  >    - 关键词1：`"{公司名}{代码} {年份}年中期分红方案每股派息"`
  >    - 关键词2：`"{公司名}{代码} {年份}年年报分红方案每股派息"`
  > 2. **按"归属年度"汇总，而非按"公告日期"汇总**：分红记录的"所属年度"字段才是正确的分组依据
  > 3. **识别双次分红公司**：近2年有中报分红记录的公司，未来中报分红概率极高，必须每次都查询中报+年报两组数据
  > 4. **验证汇总结果**：全年每股股利之和 ≈ EPS × 分红率（若≈EPS说明分红率接近100%）
  > 5. **分红率接近100%的特殊处理**：分红率>95%时，检查是否为双次分红公司，警惕将两次分红误认为一次
...
Step D：查表确定 g_floor
  ...
  | 竞争等级 | g_floor（保底增速） | 逻辑依据 |
  |---------|-------------------|---------|
  | **TOP** | **3%**（稳健）/ **4%**（极强定价权） | 品牌定价权足以抵消不投入的损失，甚至跑赢通胀；TOP-3%为基准，TOP-4%需ROE≥25%且提价历史明确 |
  | **STRONG** | **1.5%**（稳健）/ **2%**（较强惯性） | 具备行业惯性，即便不扩张也能维持现状并微增；STRONG-2%需ROE≥20% |
  | **NORMAL** | **0%** | 不投入就意味着萎缩，0 是合理的 |

Step E：最终 g 取值

  g_final = max(g_base, g_floor)

  # 实际效果：
  # - TOP 公司：g_final = max(≈0, 3-4%) = 3-4%
...
2. 选用的 g_floor 值及理由
3. 最终 g_final 取值及是否触发了 ROE 档位截断
...
**Phase 2.1 完成后 → 继续 Phase 2.5（g_final 可信度评估）**
